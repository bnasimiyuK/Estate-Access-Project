...
// ==========================================
// Get logged-in resident profile (Resident only)
// ==========================================
export const getProfile = async (req, res) => {
  try {
    ...
    if (!ResidentID) {
      return res.status(400).json({ message: "ResidentID missing in token" });
    }
...
// ==========================================
// Update logged-in resident profile (Resident only)
// ==========================================
export const updateProfile = async (req, res) => {
  try {
    const { ResidentID, role } = req.user;
    ...
    if (Password) {
      ...
      await pool
        ...
        .input("ResidentID", sql.Int, ResidentID)
        .query(`
          UPDATE Residents SET PasswordHash=@PasswordHash WHERE ResidentID=@ResidentID
        `);
    }
...
// ==========================================
// Get all residents (simple version, Admin only)
// ==========================================
export const getAllResidents = async (req, res) => {
  try {
    ...
    const pool = await sql.connect(dbConfig);
    const result = await pool.request().query("SELECT * FROM Residents");
